...
while GO == 2
    ...
    Mx = S * rand();
    My = S * rand();
    while norm([Mx, My] - G) > radius
        r = rand() * radius/2;  
        theta = 2 * pi * rand();            
        
        % Recalculate Cartesian coordinates for the monster
        Mx = r * cos(theta) + G(1);   
        My = r * sin(theta) + G(2);   
    end
    M = [Mx, My];
    ...
    viscircles(G, radius, 'Color', 'g', 'LineStyle', '--');
    xlabel('X coordinate');
    ylabel('Y coordinate');
    title('Fuzzy World Adventure (Level 1)');
    ...
    pause(0.2);  % Add a short pause for better visualization
end
